...
static int spidev_set_gpio(void)
{
	int ret;
	if (lcd_gpio == NULL) {
		debug_message();
	...
	if (ret) {
		printk(KERN_EMERG "gpio_request_array failed\n");
		return 1;
	}
	return 0;
}

static inline void spidev_unset_gpio(void)
{
	if (lcd_gpio == NULL) {
		debug_message();
	} else {
		...
		struct spi_ioc_transfer *u_xfers, unsigned n_xfers)
{
	...
	if (u_xfers->data_cmd)	/*data transfer*/
